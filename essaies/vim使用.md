...
|   输入   |                 功能描述                 |
| :------: | :--------------------------------------: |
|    i、I    | 切换为输入模式；输入位为光标所在字符「左侧、最左侧」 |
|    a、A    | 切换为输入模式；输入位为光标所在字符「右侧、最右侧」 |
|    {、}    |    命令行模式下，光标移动到「上一个空行、下一个空行」    |
|    b、w    |    命令行模式下，光标移动到「上一个单词、下一个单词」    |
| h、j、k、l | 命令行模式下，光标「左、上、下、右」移动 |
| gg、G | 命令行模式下，光标移动到「首行、尾行」 |
...
| 输入 |              功能描述              |
| :--: | :--------------------------------: |
|  x、X  |   命令行模式下，删除「光标所在字符、光标左侧字符」   |
| db、dw | 命令行模式下，删除到某「当前单词开头、下一个单词开头」 |
...
|       输入        |                     功能描述                     |
| :---------------: | :----------------------------------------------: |
|   :s/from/to/g    |       命令行模式下，替换当前行所有from为to       |
| :.,.+1s/from/to/g | 命令行模式下，替换「当前行、下一行」所有from为to |
...
### 04. [vim命令大全](https://www.dba.cn/book/vim/VIMingLingShouCe/VIVIMMingLingDaQuan.html)
...
[ 1 ] [CSDN: vi/vim替换字符](https://blog.csdn.net/qq_44930903/article/details/111105666)

[ 2 ] [维基百科: vim](https://zh.wikipedia.org/zh-cn/Vim)

[ 3 ] [vim中文用户手册](https://yianwillis.github.io/vimcdoc/doc/usr_toc.html)

[ 4 ] [vim中文手册](https://www.dba.cn/book/vim/VIMingLingShouCe/VIVIMMingLingDaQuan.html)
